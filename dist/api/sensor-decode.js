...
exports.decodeValve = decodeValve;
exports.parseSensorPayload = parseSensorPayload;
const TYPE_WIDTHS = {
    0xD8: 1, // zone state
    0xDC: 1, // hub state
 ...
 * Each zone section (comma-separated, per HA's _decode_legacy_port_section):
 *   [0] valve_state_code — lower nibble = work mode (0=idle, 1=irrigation, etc.)
 *   [1] per-port value (duration minutes when active, water volume when idle)
 *   [2] unknown
 *   [3] event_time (Unix timestamp)
 *   [4] total_duration (seconds)
 */
function decodeValveAscii(raw) {
    ...
    for (const section of zonePart.split('|')) {
        ...
        let durationSeconds = 0;
        // Field [1] is duration in minutes when valve is active
...
function parseSensorPayload(raw, model) {
    try {
        if (model === 'HCS012ARF')
            return decodeRain(raw);
        if (MOISTURE_SIMPLE_MODELS.has(model))
            return decodeMoistureSimple(raw);
        if (MOISTURE_FULL_MODELS.has(model))
            return decodeMoistureFull(raw);
        if (model.startsWith('HWS'))
...
//# sourceMappingURL=home-decoder.js.map